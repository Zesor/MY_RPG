...
void draw_dialogue_shop(rpg_t *rpg)
{
    sfRenderWindow_clear(rpg->win.window, sfTransparent);
    print_characters_shop(rpg);
...
int check_dialogue_shop(rpg_t *rpg)
{
    if (rpg->dialogue.real_dialogue == 10)
        return (1);
    return 0;
}

void left_key(rpg_t *rpg)
{
...
void event_shop_dialogue(rpg_t *rpg, sfEvent event)
{
...
void dialogue_shop(rpg_t *rpg)
{
    ...
    while (sfRenderWindow_isOpen(rpg->win.window)) {
        event_shop_dialogue(rpg, event);
        if (check_dialogue_shop(rpg) == 1)
            game_loop(rpg);
        draw_dialogue_shop(rpg);
    }
}